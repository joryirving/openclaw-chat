...
class GatewayWsManager extends EventEmitter {
  ...
  _failPendingRequests(reason = 'Gateway WS disconnected') {
  ...
  connect(origin = 'http://localhost:3000') {
    return new Promise((resolve, reject) => {
      ...
      this.ws.on('close', (code, reason) => {
        ...
        this._failPendingRequests(`Gateway WS closed (code=${code})`);
        ...
        // Always schedule reconnect unless this was an intentional disconnect.
        this._scheduleReconnect(this._lastOrigin);
      });
  ...
  disconnect() {
    ...
    this._failPendingRequests('Gateway WS manager disconnected');
